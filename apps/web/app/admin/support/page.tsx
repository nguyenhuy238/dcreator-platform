...
export default function AdminSupportPage() {
  ...
  return (
    <>
      <PageHeader title="Hàng đợi hỗ trợ" subtitle="Xử lý issue của Brand/Creator/User." action={<button className="dc-btn-secondary" onClick={() => void load()}>Làm mới</button>} />
      <section className="dc-card p-4">
        <div className="grid gap-2 md:grid-cols-4">
          ...
          <input className="dc-input" placeholder="Tìm theo tiêu đề/người gửi/email" value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>
          ...
          <div className="mt-4 grid gap-3">
            {items.map((item) => (
              <article key={item.id} className="dc-card p-4">
                ...
                <p className="mt-1 text-xs text-zinc-500">Danh mục: {item.category} • Người phụ trách: {item.assignee?.displayName ?? "Chưa phân công"}</p>
                <div className="mt-3">
                  <Link className="dc-btn-primary" href={`/admin/support/${item.id}`}>Xem chi tiết</Link>
                </div>
              </article>
            ))}
          </div>
