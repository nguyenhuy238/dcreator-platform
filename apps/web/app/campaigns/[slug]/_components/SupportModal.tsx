  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-zinc-900/45 p-4">
      <div role="dialog" aria-modal className="dc-card w-full max-w-lg p-5">
        ...
        {step === 3 ? <div className="space-y-3"><p className="text-sm font-semibold">Bước 3: Xác nhận thông tin</p><p className="text-sm text-zinc-600">Reward: <span className="font-semibold text-zinc-900">{selected?.title}</span></p><p className="text-sm text-zinc-600">Thanh toán: <span className="font-semibold text-zinc-900">{method}</span></p><p className="text-sm text-zinc-600">So tien ghi nhan: <span className="font-semibold text-zinc-900">{toSupportAmount(selected).toLocaleString("vi-VN")} VND</span></p><button className="dc-btn-primary" disabled={loading || !campaignSlug} onClick={confirmSupport}>{loading ? "Dang xu ly..." : "Xac nhan ung ho"}</button></div> : null}
        {step === 4 ? <div className="space-y-3 text-center"><p className="text-3xl">✓</p><p className="text-lg font-bold text-emerald-700">Ủng hộ thành công</p><div className="flex justify-center gap-3"><Link href="/vouchers" className="dc-btn-primary">Xem voucher của tôi</Link><button className="dc-btn-secondary" onClick={onClose}>Quay lại campaign</button></div></div> : null}
      </div>
    </div>
  );
}
