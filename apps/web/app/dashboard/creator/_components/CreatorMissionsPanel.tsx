...
function missionGroup(item: MissionItem): Exclude<MissionFilter, "ALL"> {
  ...
  if (
    item.missionApplication?.status === "PENDING_REVIEW" ||
...
export function CreatorMissionsPanel({ overview }: CreatorMissionsPanelProps) {
  ...
  const prioritizedMissions = useMemo(
    () =>
      items
        .filter((item) => missionGroup(item) !== "COMPLETED")
      ...
      {!loading ? (
        <>
          <section className="dc-grid-dashboard">
            ...
            <article className="dc-card p-4"><p className="text-sm text-zinc-600">Hoa hồng</p><p className="text-2xl font-bold">{(overview?.totalCommission ?? 0).toLocaleString("vi-VN")}đ</p></article>
          </section>
                  ...
                  <article key={item.id} className="dc-card p-4">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-sm text-zinc-500">Campaign: {item.campaign.title}</p>
                      </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button className="dc-btn-primary" onClick={() => setDetailMissionId(item.id)}>Tiếp tục làm</button>
              ...
              <div className="space-y-2">
                {filteredItems.map((item) => (
                  <article key={item.id} className="rounded-2xl border border-zinc-200 bg-white p-4">
                    <div className="grid gap-3 xl:grid-cols-[2.4fr_1fr_1fr_1.2fr_1.4fr] xl:items-center">
                      <div>
                        <p className="font-semibold text-zinc-900">{item.mission.title}</p>
                        <p className="text-sm text-zinc-500">{item.campaign.title}</p>
                      </div>
                      <div>
