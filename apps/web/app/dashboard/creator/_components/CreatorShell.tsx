...
function toBreadcrumb(pathname: string, nav: readonly CreatorNavItem[]) {
  if (pathname === "/dashboard/creator") return [{ href: "/dashboard/creator", label: "Tổng quan" }];
...
export function CreatorShell({
  ...
  const activeTitle = navItems.find((item) => isCurrent(pathname, item.href))?.label ?? "Không gian Creator";
  const userInitials = initials(user.displayName || user.email || "C");

  async function onLogout() {
    try {
      await fetch("/api/auth/logout", { method: "POST" });
    } finally {
      window.location.href = "/auth/login?next=/dashboard/creator";
    }
  }
      ...
      <div className="mx-auto flex w-full max-w-7xl">
        <aside className="sticky top-0 hidden h-screen w-80 shrink-0 border-r border-zinc-200 bg-white lg:block">
          <div className="border-b border-zinc-200 px-5 py-4">
            <p className="text-sm font-black text-zinc-900">Không gian Creator</p>
            <p className="mt-1 text-xs text-zinc-500">Quản lý nhiệm vụ, minh chứng và hoa hồng</p>
          </div>
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    aria-label="Mở thanh điều hướng creator"
                  >
                    ≡
                  </button>
                  <div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 lg:hidden" onClick={() => setMobileOpen(false)}>
          <aside className="h-full w-80 max-w-[85vw] bg-white p-3" onClick={(event) => event.stopPropagation()}>
            <div className="mb-3 flex items-center justify-between border-b border-zinc-200 pb-3">
              <p className="font-black text-zinc-900">Không gian Creator</p>
