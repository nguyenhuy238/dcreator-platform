...
import { adminApproveContentSubmission } from "@/lib/services/admin-content-review.service";
...
export async function PATCH(request: NextRequest, { params }: Props) {
  try {
    ...
    return ok(await adminApproveContentSubmission({
      actorId: actor.id,
      actorRole: actor.role,
      submissionId: id,
      feedback: payload.feedback,
      markReadyToPublish: payload.markReadyToPublish
    }));
  } catch (error) {
