...
export function CampaignProgress({ fundedAmount, targetAmount }: { fundedAmount: number; targetAmount: number }) {
  ...
  return <div><div className="mb-1 flex flex-col gap-1 text-xs text-zinc-500 sm:flex-row sm:items-center sm:justify-between"><span>{percent}% mục tiêu</span><span className="break-words">{currency(fundedAmount)} / {currency(targetAmount)}</span></div><div className="h-2 rounded-full bg-zinc-100"><div className="h-2 rounded-full bg-zinc-900" style={{ width: `${percent}%` }} /></div></div>;
}

export function CampaignCard({ campaign }: { campaign: CampaignItem }) {
  return <article className="dc-card dc-hover-lift overflow-hidden"><Image src={campaign.cover} alt={campaign.title} width={1200} height={675} className="aspect-[16/10] h-auto w-full object-cover" /><div className="space-y-3 p-4"><div className="flex flex-wrap gap-2"><StatusBadge status={campaign.status} /><span className="rounded-full border border-zinc-200 bg-zinc-50 px-2.5 py-1 text-xs font-semibold">{campaign.campaignType}</span></div><h3 className="line-clamp-2 text-lg font-bold">{campaign.title}</h3><p className="line-clamp-3 text-sm text-zinc-600">{campaign.description}</p><p className="truncate text-sm text-zinc-500">{campaign.brand} • {campaign.creator}</p><CampaignProgress fundedAmount={campaign.fundedAmount} targetAmount={campaign.targetAmount} /><div className="grid grid-cols-3 gap-2 text-xs text-zinc-500"><p>{campaign.backers} backers</p><p>{campaign.daysLeft} ngày</p><p>{campaign.rewardsLeft} rewards</p></div><Link href={`/campaigns/${campaign.slug}`} className="dc-btn-primary w-full sm:w-auto">Xem chi tiết</Link></div></article>;
}
